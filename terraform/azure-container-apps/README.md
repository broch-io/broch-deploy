...
4. Container `env { secret_name = ... }` blocks map secrets into env vars (`BROCH_LICENSE`, `BROCH_MASTER_KEY`, `ConnectionStrings__DefaultConnection`). The master key is generated by Terraform (`random_password`), not supplied — it's the at-rest encryption root the server requires at boot.
