...
4. Container `env { secret_name = ... }` blocks map secrets into env vars (`AUTHENTICATION__CLIENTSECRET`, `BROCH_MASTER_KEY`, `ConnectionStrings__DefaultConnection`, and `BROCH_LICENSE` when a key was supplied). Non-secret IdP config (`AUTHENTICATION__PROVIDER`, `CLIENTID`, `ADMINROLES`, …) is passed as plain `env`. The master key is generated by Terraform (`random_password`), not supplied — it's the at-rest encryption root the server requires at boot. The license is optional: leave `broch_license` unset and the server boots unlicensed for in-app activation.
