...
> ⚠️ **The break-glass password rotates on every redeploy** (except `reuseExistingSecrets=true`). `vmPasswordSeed` defaults to a fresh value each `az deployment group create`, so the `vm-admin-password` secret is overwritten on every run — even a routine redeploy to bump `brochVersion` or change a DNS provider. Any out-of-band copy of the old password stops working; re-read it from the break-glass vault after each deploy. To keep a stable password across redeploys, pass an explicit `vmPasswordSeed` (and store it yourself). A `reuseExistingSecrets=true` retry (below) deliberately does **not** rewrite the secret: Azure ignores an `adminPassword` change on an existing VM, so rewriting would rotate the vault copy away from the password the VM actually has.
...
## Retrying a failed deployment (`reuseExistingSecrets`)

A deployment that fails partway (quota, capacity, a permissions error on the DNS role assignment) leaves the already-created resources — including the Key Vaults with every secret this run supplied — in the resource group. **Do not start over in a new resource group, and do not use the portal's Redeploy button** (it blanks all secureString inputs). Instead: fix the cause (e.g. request the quota increase the error links), then run the **same deployment into the same resource group** with the vault-backed secrets left **empty**:

```sh
az deployment group create -g $RG -f main.bicep \
  --parameters reuseExistingSecrets=true \
  --parameters vmName=... dnsZone=... <same non-secret selections as the failed attempt> ...
```

What `reuseExistingSecrets=true` changes:

- **Blank means "keep the stored value"** for the vault-backed secrets — `brochMasterKey`, `authClientSecret`, the DNS credentials, `databaseConnectionString`, `localDbAdminPassword`. They survive the failed attempt in the app vault (deterministic names — the retry re-targets the same vaults), and blank params no longer risk overwriting them.
- **The break-glass `vm-admin-password` secret is not rotated** (see the warning above) — the vault copy keeps matching the VM. Two divergence edges, both recoverable with `az vm user update -g <rg> -n <vmName> -u broch -p <new>`: (a) the first attempt failed so early that the break-glass secret was never written — the retried VM's password is then stored nowhere; (b) `reuseExistingSecrets=true` was mistakenly used on a genuinely FIRST deploy (no secret written, VM got password #1), then corrected with a normal redeploy — that rewrite stores password #2 while the existing VM keeps #1 (Azure ignores the change). If you ever need Serial Console and the vault password doesn't work, reset it.
- The boot-fetch list is derived from the selected modes/providers rather than from which params this run supplied, so a retry where the failure landed **between** the secret writes and the VM still boots with the full secret set. (A failure so early that the secrets themselves were never written can't be reuse-retried — the boot-fetch halts on the missing secrets, fails closed; retry that shape as a normal first deploy with values.)

What you must still re-supply, because it lives in `customData` (not the vault — see "Still in `customData`" above) or is a live resource property:

- **BYO TLS cert + key** (`certMode=Byo`), the **GCP service-account JSON**, and any **private-registry token** — verbatim.
- **Managed mode: the SAME `postgresAdminPassword`** as the failed attempt. It is a property on the flexible server itself; supplying a different value on a retry where the server already exists would reset the server password while an already-booted VM keeps the old connection string.

And repeat the **same non-secret selections** (region, modes, provider, names) as the failed attempt — the retry is the same deployment, resumed, not a chance to change shape. First deployments must keep `reuseExistingSecrets=false` (the default): with no stored secrets to reuse, a blank-secret "retry" into a fresh RG halts at the boot-fetch (fails closed) rather than booting half-configured.

One caveat: reuse assumes the first attempt supplied an OAuth client secret whenever `authProvider` is set — the marketplace wizard always does. A **secretless**-auth deployment (public-client/PKCE-style, no `authClientSecret`) must not use `reuseExistingSecrets`: its retry would list an `auth-client-secret` that was never stored and halt at the boot-fetch. Retry that shape as a normal deploy with values.

The Azure Marketplace wizard exposes this as the "Redeploy into the same resource group" deployment type.
